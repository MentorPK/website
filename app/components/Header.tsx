...
const GlowLink = ({ text, to = '' }) => {
  return (
    <div className="relative group group-hover:scale-125">
      ...
      <Link
        className="relative bg-black border-2 border-white py-[6px] px-[12px] rounded-3xl text-lg opacity-90 group-hover:opacity-100 transition ease-in-out duration-200"
        to={to}
      >
        {text}
      </Link>
...
const Header = ({ clicked }: HeaderInterface) => {
  return (
    <div
      className={`w-full bg-black fixed top-0 transition-header duration-250 ease-in-out shadow-xl ${
